...
%% PREAMBLE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\Sexpr{knit_child("cv_preamble.tex")}

%% R FUNCTIONS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\Sexpr{knit_child("cv_functions.Rnw")}
...
\vspace{0.1in}
\noindent
\textbf{Assistant Scientist, Global Food Safety and Zoonoses, PhD DVM MVSc MStat}\\
\textit{Emerging Pathogens Institute \& Department of Animal Sciences, University of Florida}\\
\textit{Center for Burden and Risk Assessment---\href{http://www.cbra.be}{www.cbra.be}}\\
\href{mailto:bdevleesschauwer@ufl.edu}{bdevleesschauwer@ufl.edu} $\mid$ \href{http://animal.ifas.ufl.edu/faculty/devleesschauwer/}{animal.ifas.ufl.edu/faculty/devleesschauwer} $\mid$ +1 352 392 1981\\
2250 Shealy Drive, Gainesville, FL 32611, USA $\mid$ PO Box 110910
...
\begin{itemize}
  ...
  \item Epidemiology, burden of disease, risk assessment, health economics;
  \item Bayesian data analysis, true prevalence estimation, meta-analysis, disease mapping;
...
\begin{itemize}
  ...
  \item \textbf{Assistant scientist}, 06/2015--present \hfill \\
...
wb <- loadWorkbook("short_courses.xlsx")
db <- readWorksheet(wb, sheet = "index")

db <- db[order(db$title), ]
db <- db[order(db$year, decreasing = TRUE), ]

for (i in seq(nrow(db))) {
  cat("\\item \\textbf{", db$title[i], "}, ", db$year[i], " \\hfill \\\\ \n", sep = "")
  cat(gsub("é", "\\\\'{e}", db$location[i]), "\n")
}
@
\end{itemize}
...
\begin{itemize}
  \setlength\itemsep{0em}
  \item \textbf{Associate Editor}, 2015--present\hfill \\
    BMC Veterinary Research, Parasitology section
  \item \textbf{Reviewer for international scientific journals}, e.g.:\hfill \\
    Acta Tropica, Advances in Parasitology, BMC Infectious Diseases, BMJ Case Reports, Bulletin of the World Health Organization, Health Affairs, International Journal of Environmental Research and Public Health, Journal of Neurosciences in Rural Practice, Parasitology International, PLOS One, Risk Analysis, Transactions of the Royal Society of Tropical Medicine \& Hygiene, Vector-Borne and Zoonotic Diseases
\end{itemize}

\subsection{Participation in international projects}
\begin{itemize}
  \item \textbf{Risk forecasting workgroup vice-leader}, 2015--present \hfill \\
    EU COST action FA1408 EURO-FBP
  \item \textbf{Epidemiology workgroup leader}, 2013--present \hfill \\
    EU COST action TD1302 CYSTINET---\href{http://www.cystinet.org}{www.cystinet.org}
  
  \item \textbf{Computational Task Force member}, 2011--present\hfill \\
    \textbf{Country Studies Task Force member}, 2011--present\hfill \\
    Foodborne Disease Burden Epidemiology Reference Group, WHO
\end{itemize}

\subsection{Participation in international development projects}
\begin{itemize}
  \item \textbf{USAID Feed the Future Innovation Lab on Livestock Systems}, 2015--2020\hfill \\
  ...
  \item \textbf{VLIR-UOS Institutional University Cooperation with Jimma University, Ethiopia}, 2012--2016\hfill \\
...
\begin{itemize}
  \item Expert consultation to discuss preliminary results of the joint WHO/IHME analysis of causes of death among children aged 5--14 years in the WHO European Region. Copenhagen, Denmark; 2015.
  ...
  \item FAO/WHO Expert Meeting on Risk-based examples for Control of \emph{Trichinella} spp. and \emph{Taenia saginata} in meat. Geneva, Switzerland; 2013--2014.
\end{itemize}

\subsection{Logistic responsibilities}
\subsubsection{Management of research centers}
\begin{itemize}
  \item \textbf{Founding member}, 2014--present\hfill \\
  Center for Burden and Risk Assessment, \href{http://www.cbra.be/}{www.cbra.be}
\end{itemize}

\subsubsection{Organisation of national and international conferences}
\begin{itemize}
  \item \textbf{Member of scientific committee}\hfill \\
    1st International CYSTINET Conference, 3--4 November 2015, Belgrade, Serbia
\end{itemize}

\subsubsection{Other}
\begin{itemize}
  \item \textbf{Event organizer}, 2012--2014\hfill \\
    RBelgium, the Belgian R user group
  \item \textbf{Rapporteur}, November 2010\hfill \\
    Fourth meeting of the Foodborne Disease Burden Epidemiology Reference Group\hfill \\
...
\begin{itemize}
  \setlength\itemsep{-0.1em}
  \item \textbf{Statistical software}: R, WinBUGS, OpenBUGS, JAGS, SAS, Stata
  \item \textbf{Advanced R skills}: package development, Tcl/Tk interfaces, Shiny applications
  \item \textbf{Design and programming}: html, javascript, php, mysql, \LaTeX, C++
...
\subsection{University lectures}
\begin{itemize}
  \setlength\itemsep{0em}
  ...
  \item \textbf{Topics in tropical veterinary medicine}, 2013--2015 \hfill \\
...
\subsection{Promoter of 4 short-term scientific missions}
\begin{itemize}
  \setlength\itemsep{0em}
  ...
  \item \textbf{Dr Hardy Richter}, September 2014 \hfill \\
...
cat("\\subsection{(Co)supervisor of",
    nrow(db_msc), "MSc students}\n")
...
gs <- get_profile(id)
@
\textbf{Google Scholar metrics: \Sexpr{gs$total_cites} citations $\cdot$ h-index \Sexpr{gs$h_index} $\cdot$ i10-index \Sexpr{gs$i10_index}}
...
\begin{itemize}
  \setlength\itemsep{0em}
  \item[{[5]}] World Health Organization (2015) WHO estimates of the global burden of foodborne diseases. Foodborne diseases burden epidemiology reference group 2007-2015. Geneva: WHO Press.
  ...
  \item[{[3]}] Food and Agriculture Organization of the United Nations, World Health Organization (2014) Risk based examples for control of \emph{Trichinella} spp. and \emph{Taenia saginata} in meat. Report of a joint FAO/WHO expert meeting, 22--25 October 2013, WHO Headquarters, Geneva, Switzerland.
  \item[{[2]}] World Health Organization (2014) WHO initiative to estimate the global burden of foodborne diseases: fifth formal meeting of the Foodborne Disease Burden Epidemiology Reference Group (FERG), 8--12 April 2013, Geneva, Switzerland. Geneva: WHO Press.
  \item[{[1]}] World Health Organization (2014) WHO initiative to estimate the global burden of foodborne diseases: fourth formal meeting of the Foodborne Disease Burden Epidemiology Reference Group (FERG): Sharing New Results, Making Future Plans, and Preparing Ground for the Countries. Geneva: WHO Press.
\end{itemize}

\subsection{R Packages}
\begin{itemize}
  \setlength\itemsep{0em}
  \item[{[2]}] \textbf{Devleesschauwer B},  Torgerson P, Charlier J, Levecke B, Praet N, Roelandt S, Smit GSA, Dorny P, Berkvens D, Speybroeck N (2015). prevalence: Tools for Prevalence Assessment Studies. R package version 0.4.0. \url{http://cran.r-project.org/package=prevalence}
  \item[{[1]}] \textbf{Devleesschauwer B}, McDonald S, Haagsma A, Praet N, Havelaar A, Speybroeck N (2014) DALY: The DALY Calculator---A GUI for stochastic DALY calculation in R. R package version 1.4.0. \url{http://cran.r-project.org/package=DALY}
\end{itemize}

\subsection{Vulgarizing articles}
\begin{itemize}
  \setlength\itemsep{0em}
  \item[{[4]}] Havelaar AH, \textbf{Devleesschauwer B}. \textit{One in 10 globally suffer from foodborne diseases, WHO study finds.} EPI News, 3 December 2015.
  \item[{[3]}] Speybroeck N, Maertens de Noordhout C, \textbf{Devleesschauwer B}. \textit{Des nombres pour des aliments s\^{u}rs.} La Libre Belgique, 12 April 2015.
  \item[{[2]}] Speybroeck N, Maertens de Noordhout C, \textbf{Devleesschauwer B}. \textit{Comment les nombres rendent vos aliments plus s\^{u}rs.} Le Soir, 8 April 2015.
  \item[{[1]}] Speybroeck N, Maertens de Noordhout C, \textbf{Devleesschauwer B}. \textit{Hoe getallen je voedsel veiliger maken.} De Morgen, 7 April 2015.
\end{itemize}
